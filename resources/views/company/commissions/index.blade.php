<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('commission.page.company_index') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="GET" class="bg-white shadow sm:rounded-lg p-4 flex flex-wrap gap-3 items-end">
                <div>
                    <x-input-label for="user_id" :value="__('commission.filters.staff')" />
                    <select id="user_id" name="user_id" class="mt-1 border-gray-300 rounded-md text-sm">
                        <option value="">{{ __('commission.filters.all_staff') }}</option>
                ...
                @if (request()->hasAny(['user_id', 'status', 'from', 'to']))
                    <a href="{{ route('company.commissions.index') }}"
                       class="text-sm text-gray-600 hover:underline pb-2">
                        {{ __('commission.actions.reset') }}
                    </a>
                @endif
            </form>
                    ...
                    <div class="p-5 flex items-center justify-between gap-6">
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                <h3 class="font-medium text-gray-900">{{ $commission->user->name }}</h3>
                            ...
                            <p class="text-sm text-gray-600 mt-1">{{ $commission->jobPost->title }}</p>
                        ...
                        <div class="flex items-center gap-4 shrink-0">
                            ...
                            @can('manage', $commission)
                                <div class="flex items-center gap-2 text-sm">
                                    @foreach ($commission->status->allowedTransitions() as $target)
                                        <form method="POST" action="{{ route($target->routeName(), $commission) }}">
                                            @csrf
                                            @method('PATCH')
